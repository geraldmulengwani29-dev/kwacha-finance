...
const ClientProfile = () => {
  ...
  const handlePrint = () => {
    ...
    const loansRows = loans.map(loan => {
      ...
      return `<tr><td>K${loan.amount}</td><td>${escapeHTML(loan.collateral)}</td><td>${weeks}</td><td>K${totalOwed.toFixed(2)}</td><td style="color:#27ae60">K${paid.toFixed(2)}</td><td style="color:${remaining > 0 ? '#e74c3c' : '#27ae60'}">${remaining > 0 ? `K${remaining.toFixed(2)}` : '✔ Cleared'}</td><td>${loan.status}</td></tr>`;
    }).join('');
    ...
    const appsRows = applications.map(app => `<tr><td>K${app.requestedAmount}</td><td>${escapeHTML(app.collateral)}</td><td>${app.status}</td><td>${app.offeredAmount ? `K${app.offeredAmount}` : '-'}</td><td>${formatDate(app.createdAt)}</td></tr>`).join('');
