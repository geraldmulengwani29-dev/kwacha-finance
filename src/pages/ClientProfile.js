...
const ClientProfile = () => {
  ...
  const handlePrint = () => {
    ...
    const loansRows = loans.map(loan => {
      const weeks = getWeeksPassed(loan.startDate);
      const weeksEscaped = escapeHTML(weeks);
      const totalOwed = calculateTotalOwed(loan);
      const paid = getTotalPaid(loan.id);
      const remaining = Math.max(0, totalOwed - paid);
      return `<tr><td>K${escapeHTML(loan.amount)}</td><td>${escapeHTML(loan.collateral)}</td><td>${weeksEscaped}</td><td>K${totalOwed.toFixed(2)}</td><td style="color:#27ae60">K${paid.toFixed(2)}</td><td style="color:${remaining > 0 ? '#e74c3c' : '#27ae60'}">${remaining > 0 ? `K${remaining.toFixed(2)}` : '✔ Cleared'}</td><td>${escapeHTML(loan.status)}</td></tr>`;
    }).join('');
