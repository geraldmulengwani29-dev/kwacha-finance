...
const ClientProfile = () => {
  ...
  const handlePrint = () => {
    ...
    const loansRows = loans.map(loan => {
      ...
      return `<tr><td>K${escapeHTML(loan.amount)}</td><td>${escapeHTML(loan.collateral)}</td><td>${escapeHTML(weeks)}</td><td>K${escapeHTML(totalOwed.toFixed(2))}</td><td style="color:#27ae60">K${escapeHTML(paid.toFixed(2))}</td><td style="color:${remaining > 0 ? '#e74c3c' : '#27ae60'}">${remaining > 0 ? `K${escapeHTML(remaining.toFixed(2))}` : '✔ Cleared'}</td><td>${escapeHTML(loan.status)}</td></tr>`;
    }).join('');
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <p class="meta">Generated on ${escapeHTML(new Date().toLocaleDateString('en-GB', { day:'numeric', month:'long', year:'numeric' }))} &bull; Kwacha Finance</p>
        ...
        <div class="stats-grid">
          <div class="stat-card"><h3>Total Borrowed</h3><p style="color:#c9a84c">K${escapeHTML(totalBorrowed.toLocaleString())}</p></div>
          <div class="stat-card"><h3>Total Paid</h3><p style="color:#27ae60">K${escapeHTML(totalPaid.toLocaleString())}</p></div>
          <div class="stat-card"><h3>Active Loans</h3><p style="color:#c9a84c">${escapeHTML(loans.filter(l => l.status === 'active').length)}</p></div>
          <div class="stat-card"><h3>Completed Loans</h3><p style="color:#27ae60">${escapeHTML(loans.filter(l => l.status === 'completed').length)}</p></div>
        </div>
        <h2>Loans</h2>
