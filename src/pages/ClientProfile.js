...
const ClientProfile = () => {
  ...
  const handlePrint = () => {
    ...
    const loansRows = loans.map(loan => {
      ...
      // Defensive coding: sanitize all dynamic content to prevent XSS in manual HTML construction
      return `<tr><td>K${escapeHTML(loan.amount)}</td><td>${escapeHTML(loan.collateral)}</td><td>${weeks}</td><td>K${totalOwed.toFixed(2)}</td><td style="color:#27ae60">K${paid.toFixed(2)}</td><td style="color:${remaining > 0 ? '#e74c3c' : '#27ae60'}">${remaining > 0 ? `K${remaining.toFixed(2)}` : '✔ Cleared'}</td><td>${escapeHTML(loan.status)}</td></tr>`;
    }).join('');
    const paymentsRows = payments.map(p => `<tr><td style="color:#27ae60">K${escapeHTML(p.amount)}</td><td>${formatDate(p.date)}</td><td>${escapeHTML(p.notes) || '-'}</td></tr>`).join('');
    const appsRows = applications.map(app => `<tr><td>K${escapeHTML(app.requestedAmount)}</td><td>${escapeHTML(app.collateral)}</td><td>${escapeHTML(app.status)}</td><td>${app.offeredAmount ? `K${escapeHTML(app.offeredAmount)}` : '-'}</td><td>${formatDate(app.createdAt)}</td></tr>`).join('');
